...
**`sprtTrgtAgeLmtYn` 은 이름과 뜻이 반대다.** "연령 제한 여부"가 아니라 **"연령 무관 여부"** 다. `Y`(1,219건)가 연령 제한 없음이고 그중 631건은 범위가 `0~0` 이다. `N`(1,474건)이 연령 제한 있음이고 `19~39` 같은 실제 범위를 갖는다. 반대로 읽으면 25세 사용자에게 `min_age 29` 정책이 걸린다.
